...
use super::{
    funcs::{get_node_index, remove_from_supernode, to_single_node},
    intersects,
};
...
fn handle_deletion(
    ...
    if let Some(_p_sub) = metadata.nodes.get(&triple.sub).unwrap().parent {
        ...
        // Handle a split of a supernode if no intersection has been found
        split_supernode(metadata, &_p_sub);
        // If a supernode only consists of a single node, then remove clique, else do nothing
        handle_singleton_supernodes(metadata, source_clique, target_clique, &_p_sub);
        // Remove if edges of sub or obj are empty
        handle_empty_edge(metadata, &triple.sub, source_clique, true);
        // Remove node from clique
        remove_preds_in_clique(metadata, triple, source_clique, true);
        // Creates new cliques if necessary
        create_cliques(metadata, &triple.sub, source_clique, true);
        ...
        // Remove if edges of sub or obj are empty
        handle_empty_edge(metadata, &triple.sub, source_clique, is_sub);
        // Remove node from clique
        remove_preds_in_clique(metadata, triple, source_clique, is_sub);
        // Creates new cliques if necessary
        create_cliques(metadata, &triple.sub, source_clique, is_sub);
    }
    ...
    if let Some(_p_obj) = metadata.nodes.get(&triple.obj).unwrap().parent {
        ...
        split_supernode(metadata, &_p_obj);
        ...
        remove_edge(&mut metadata.nodes, triple, is_sub);
        // Remove if edges of sub or obj are empty
        handle_empty_edge(metadata, &triple.obj, target_clique, is_sub);
        // Remove node from clique
        remove_preds_in_clique(metadata, triple, target_clique, is_sub);
        // Creates new cliques if necessary
        create_cliques(metadata, &triple.obj, target_clique, is_sub);
    }
}
...
fn split_supernode(metadata: &mut MetaData, node: &u32) {
    ...
    if !intersects(&node_inc_preds, &rest_inc_preds)
        && !intersects(&node_out_preds, &rest_out_preds)
    {
...
fn handle_nodes(metadata: &mut MetaData, triple: &Triple, clique: &mut Vec<Clique>, is_sub: bool) {
...
fn handle_empty_edge(metadata: &mut MetaData, node: &u32, clique: &mut Vec<Clique>, is_sub: bool) {
    let nodeinfo = metadata.nodes.get(node).unwrap();
    ...
    if is_edge_empty(nodeinfo, is_sub) {
        ...
        if nodeinfo.parent.is_none() {
            //remove the node from the clique
            clique.remove(c_index);
            //add the node to the empty clique
            clique[0].add_node(&(c_index as u32));
        } else {
            split_supernode(metadata, node);
            //extract the node from the supernode in clique
            /*
            let supernode_id = nodeinfo.parent.unwrap();
            let snode = metadata.supernodes.get(&supernode_id).unwrap().clone();
            let c_index = metadata.index_map.get(node).unwrap()[clique_type];
            let snode_index = metadata.index_map.get(&snode[0]).unwrap()[clique_type];
            clique.remove(c_index);
            //add the node to the empty clique
            clique[0].add_node(&(c_index as u32));*/       
        }
...
/// Group intersected nodes into a clique and removes the original clique
...
fn create_cliques(
    ...
    let resulting_cliques = check_for_new_cliques(metadata, node, clique, is_sub);
...
fn remove_edge(nodes: &mut HashMap<u32, NodeInfo>, triple: &Triple, is_sub: bool) {
    let node = if is_sub { triple.sub } else { triple.obj };
    if is_sub {
        nodes
            .get_mut(&node)
...
fn get_preds_in_edges(edges: &Vec<Vec<u32>>) -> Vec<u32> {
...
fn get_edge_preds(metadata: &MetaData, node: &u32, is_sub: bool) -> Vec<u32> {
    let edge_preds = if is_sub {
        get_preds_in_edges(&metadata.nodes.get(node).unwrap().outgoing)
    } else {
        get_preds_in_edges(&metadata.nodes.get(node).unwrap().incoming)
    };
    return edge_preds;
}

/// Group all outgoing (true) or incoming (false) edges into one vector
fn get_all_edges(nodeinfo: &NodeInfo, is_sub: bool) -> Vec<u32> {
    let mut edges: Vec<u32> = Vec::new();
    let nodeinfo_edge = if is_sub {
        &nodeinfo.outgoing
    } else {
        &nodeinfo.incoming
    };
...
fn has_parent(node: &u32, metadata: &MetaData) -> bool {
